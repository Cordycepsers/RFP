...
class ProposalandScheduler:
    ...
    def __init__(self, config_path: str = "config/proposaland_config.json"):
        ...
        self.daily_time = self.config.get(
            'scheduler', {}).get('daily_time', '09:00')
        self.timezone = self.config.get('scheduler', {}).get('timezone', 'UTC')
        self.max_retries = self.config.get(
            'scheduler', {}).get('max_retries', 3)
        self.retry_delay = self.config.get(
            'scheduler', {}).get('retry_delay_minutes', 30)
    ...
    def run_daily_monitoring(self):
        ...
        try:
            ...
            filtered_opportunities = self.monitor.filter_opportunities(
                opportunities)
            scored_opportunities = self.monitor.score_opportunities(
                filtered_opportunities)
            classified_opportunities = self.monitor.classify_priorities(
                scored_opportunities)

            # Convert OpportunityData objects to dicts for output and notification modules
            classified_opportunities_dicts = [
                opp.__dict__ for opp in classified_opportunities]

            # Step 3: Generate outputs
            logger.info("Step 3: Generating Excel and JSON outputs")
            excel_path = self.excel_generator.generate_tracker(
                classified_opportunities_dicts)
            json_path = self.json_generator.generate_json_output(
                classified_opportunities_dicts)

            # Step 4: Generate summary
            logger.info("Step 4: Generating summary report")
            summary = self.monitor.generate_summary_report(
            ...
            critical_opportunities = [
                opp for opp in classified_opportunities_dicts if opp.get('priority') == 'Critical']
            if critical_opportunities:
                logger.info(
                    f"Step 6: Sending urgent alert for {len(critical_opportunities)} critical opportunities")
                self.email_notifier.send_urgent_alert(critical_opportunities)

            # Log completion
            duration = datetime.now() - start_time
            logger.info(
                f"Daily monitoring completed successfully in {duration}")
            ...
            self._save_execution_summary(
                classified_opportunities, summary, duration, True)
    ...
    def run_with_retry(self):
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.info(
                    f"Daily monitoring attempt {attempt}/{self.max_retries}")
    ...
    def start_scheduler(self):
        ...
        logger.info(
            f"Starting Proposaland scheduler - daily execution at {self.daily_time}")
    ...
    def _send_empty_report(self):
        ...
        try:
            ...
            self.email_notifier.send_daily_report(
                empty_opportunities, summary, excel_path, json_path)
...
def main():
    ...
    try:
        ...
        if args.run_once:
            ...
            # Enter the scheduler run loop so the process stays alive when started
            # by the start script or via nohup.
            scheduler.run()
